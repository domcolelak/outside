...
export function generateIntelFindings(assets: Asset[], now: string): Finding[] {
  ...
  for (const asset of assets) {
    ...
    out.push({
      ...
      textKey: "adverseReputation",
      textValues: {
        label: asset.label,
        ip,
        source,
        score,
        reports: reports ? ` from ${reports} report(s)` : "",
        lastReported: lastReported ? ` (last reported ${lastReported.slice(0, 10)})` : "",
      },
  ...
  for (const asset of assets) {
    ...
    out.push({
      ...
      assetId: asset.id,
      category: "threat-intelligence",
      textKey: "maliciousAddress",
      textValues: {
        label: asset.label,
        ip,
        name: name ? ` (${name})` : "",
        lastSeen: lastSeen ? ` (last seen ${lastSeen.slice(0, 10)})` : "",
      },
  ...
  if (root && vtFlags > 0) {
    ...
    out.push({
      ...
      assetId: root.id,
      category: "threat-intelligence",
      textKey: "domainFlagged",
      textValues: {
        source,
        malicious,
        label: root.label,
        suspicious: suspicious ? ` and ${suspicious} as suspicious` : "",
      },
  ...
  if (root && breachCount > 0) {
    ...
    out.push({
      ...
      textKey: "breachExposure",
      textValues: {
        source,
        count: breachCount,
        label: root.label,
        latest: latest ? `, most recent dated ${latest}` : "",
        names: names.length ? `: ${names.slice(0, 5).join(", ")}${names.length > 5 ? "…" : ""}` : "",
      },
