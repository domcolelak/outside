...
import { recordScanPersistenceFailure } from "@/lib/observability/metrics";

export class ScanPersistenceError extends Error {
  readonly code?: string;

  constructor(cause: unknown) {
    super("Required scan persistence failed", { cause });
 ...
 * from real history (not naming heuristics). Ad-hoc anonymous callers may use
 * best-effort persistence; monitored/authenticated paths must request strict
 * mode so a missing history row can never be presented as a successful scan.
 */
export async function recordScan(
  ...
  try {
    ...
    const snapshots = result.graph.assets.map((a) =>
      toSnapshot(a, result.scanId, ""),
    );
    const { previousScanId, previousSnapshots, seenBefore } =
      await store.saveScan(target, result, snapshots);
    ...
    recordScanPersistenceFailure(throwOnError);
    if (throwOnError) throw new ScanPersistenceError(err);
    console.warn(
      "[persistence] recordScan failed (non-fatal):",
