...
export function generateMisconfigurationFindings(assets: Asset[], now: string): Finding[] {
  ...
  for (const asset of assets) {
    ...
    if (missing.length > 0) {
      out.push({
        ...
        textKey: "missingHeaders",
        textValues: { label, count: missing.length, missing: missing.join(", ") },
    ...
    if (/^http:\/\//i.test(redirect)) {
      out.push({
        ...
        textKey: "httpsDowngrade",
        textValues: { label, redirect: redirect.slice(0, 120) },
    ...
    if (certDays !== null && certDays <= 30) {
      ...
      out.push({
        ...
        textKey: certDays <= 0 ? "certExpired" : "certExpiring",
        textValues: {
          label,
          days: certDays,
          notAfter: typeof asset.attrs.certNotAfter === "string" ? ` (not-after ${asset.attrs.certNotAfter.slice(0, 10)})` : "",
        },
  ...
  if (root && domainDays !== null && domainDays <= 45) {
    out.push({
      ...
      textKey: domainDays <= 0 ? "domainLapsed" : "domainExpiring",
      textValues: {
        label: root.label,
        days: domainDays,
        expiresAt: typeof root.attrs.domainExpiresAt === "string" ? ` (${root.attrs.domainExpiresAt.slice(0, 10)})` : "",
      },
