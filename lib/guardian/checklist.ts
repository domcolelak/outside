...
export function evaluateChecklist(result: ScanResult): GuardianChecklistItem[] {
  ...
  const primary = result.graph.assets.find((asset) => asset.attrs.https === "observed")
    ?? result.graph.assets.find((asset) => typeof asset.attrs.certDaysToExpiry === "number")
  ...
  const rows: GuardianChecklistItem[] = [
    ...
    item(result, { code: "https", label: "HTTPS", state: httpsObserved ? "pass" : "unknown", asset: primary, observation: httpsObserved ? "The primary surface was observed over HTTPS with certificate validation." : "A validated HTTPS response was not observed for the primary surface; Guardian does not infer absence from a transport failure.", explanation: "HTTPS authenticates the service and encrypts traffic in transit.", whyItMatters: "Public web and authentication traffic should not traverse plaintext channels.", recommendedAction: "Provision a trusted certificate, redirect HTTP to HTTPS, and remove mixed-content dependencies." }),
