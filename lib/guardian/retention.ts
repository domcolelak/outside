...
export async function runGuardianRetention(now = new Date(), batchSize = 2_000, maxBatches = 10): Promise<RetentionRunResult> {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    await drain(() => tx.$executeRaw(Prisma.sql`WITH doomed AS (SELECT item."id" FROM "guardian_digests" item JOIN "guardian_retention_policies" policy ON policy."orgId" = item."orgId" WHERE item."generatedAt" < ${now} - policy."digestDays" * INTERVAL '1 day' ORDER BY item."generatedAt" LIMIT ${batchSize} FOR UPDATE OF item SKIP LOCKED) DELETE FROM "guardian_digests" item USING doomed WHERE item."id" = doomed."id"`), "digests");
    await drain(() => tx.$executeRaw(Prisma.sql`WITH doomed AS (SELECT item."id" FROM "scans" item JOIN "guardian_retention_policies" policy ON policy."orgId" = item."orgId" WHERE item."finishedAt" < ${now} - policy."scanDays" * INTERVAL '1 day' ORDER BY item."finishedAt" LIMIT ${batchSize} FOR UPDATE OF item SKIP LOCKED) DELETE FROM "scans" item USING doomed WHERE item."id" = doomed."id"`), "scans");
