...
function appearedEvent(snapshot: GuardianSnapshot, change: ChangeEvent, current?: GuardianInventoryItem): GuardianEvent {
  ...
  if (current?.isNonProduction) return makeEvent(snapshot, { ...common, type: "nonproduction_reachable", category: "surface", severity: "high", confidence: 0.82, title: "New non-production surface is publicly reachable", summary: `${change.label} is newly observable and its hostname or public response contains a non-production signal.`, why: "Staging and development systems often have different ownership, data, and access-control expectations than production.", groupKey: `nonprod-new:${change.canonical}` });
...
export function correlateGuardianEvents({ current, previous, history, changes: suppliedChanges = [] }: CorrelationInput): GuardianEvent[] {
  ...
  if (previous && changes.length === 0) {
    ...
    for (const item of current.inventory) {
      ...
      if (!old) changes.push({ type: everSeen.has(item.canonical) ? "asset_returned" : "asset_appeared", canonical: item.canonical, label: item.label, detail: everSeen.has(item.canonical) ? "A previously observed asset is publicly reachable again after being absent." : "A new public asset appeared on the external surface.", priority: item.priority });
      else {
    ...
    for (const item of previous.inventory) if (!now.has(item.canonical)) changes.push({ type: "asset_disappeared", canonical: item.canonical, label: item.label, detail: "An asset observed in the previous scan is no longer publicly reachable.", priority: "low" });
  }
  ...
  for (const [canonical, item] of now) {
    ...
    if (!old) continue;
    if (old.addresses.join("|") !== item.addresses.join("|")) events.push(makeEvent(current, { type: "dns_changed", category: "infrastructure", severity: "medium", confidence: 1, title: "DNS destination changed", summary: `${item.label} resolves to a different public address set.`, why: "A changed destination can reflect a deployment, provider migration, failover, or unexpected DNS modification.", assets: [canonical], observations: [`Previous addresses: ${describeList(old.addresses)}.`, `Current addresses: ${describeList(item.addresses)}.`], groupKey: `dns:${canonical}` }));
    ...
    if (oldInfra !== newInfra) events.push(makeEvent(current, { type: "infrastructure_changed", category: "infrastructure", severity: "medium", confidence: 0.95, title: "Infrastructure provider signals changed", summary: `${item.label} has different publicly observable provider signals.`, why: "Provider changes are strong indicators of cloud, DNS, mail, or delivery migration and should match planned work.", assets: [canonical], observations: [`Previous: DNS ${old.dnsProvider ?? "unknown"}, cloud ${old.cloudProvider ?? "unknown"}, CDN ${old.cdn ?? "unknown"}, mail ${old.mailProvider ?? "unknown"}.`, `Current: DNS ${item.dnsProvider ?? "unknown"}, cloud ${item.cloudProvider ?? "unknown"}, CDN ${item.cdn ?? "unknown"}, mail ${item.mailProvider ?? "unknown"}.`], groupKey: `infrastructure:${canonical}` }));
