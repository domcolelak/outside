...
import type { DiscoveryMethod, Finding, Priority, ProviderRun } from "@/lib/types";

export const ASSESS_CATALOGUE_VERSION = "2026-07-29";
...
export interface AssessCheck {
  ...
  category: string;
  /** Provider methods that must have completed fully before absence can pass. */
  requires: DiscoveryMethod[];
...
export const ASSESS_CHECKS: readonly AssessCheck[] = [
  {
    ...
    category: "certificate-expiry", requires: ["http_observation"], mode: "verified-safe",
  ...
  },
  {
    ...
    category: "security-headers", requires: ["http_observation"], mode: "verified-safe",
  ...
  },
  {
    ...
    category: "insecure-redirect", requires: ["http_observation"], mode: "verified-safe",
  ...
  },
  {
    ...
    category: "mail-security", requires: ["dns_mx"], mode: "verified-safe",
  ...
  },
  {
    ...
    category: "known-vulnerability", requires: ["http_observation"], mode: "verified-safe",
  ...
  },
  {
    ...
    category: "exposed-service", requires: ["service_observation"], mode: "verified-safe",
  ...
  },
  {
    ...
    category: "auth-surface", requires: ["certificate_transparency", "dns"], mode: "verified-safe",
  ...
  },
  {
    ...
    category: "non-production-exposure", requires: ["certificate_transparency", "dns"], mode: "verified-safe",
  ...
  },
  {
    ...
    category: "shadow-asset", requires: ["certificate_transparency", "dns"], mode: "verified-safe",
  ...
  },
  {
    ...
    category: "infrastructure-concentration", requires: ["certificate_transparency", "dns", "dns_mx"], mode: "verified-safe",
  ...
  },
  {
    ...
    category: "domain-expiry", requires: ["domain_registration"], mode: "verified-safe",
...
export interface AssessCheckResult {
  ...
  findingIds: string[];
  /** Why the detector could not make a reliable pass/fail decision. */
  reason?: string;
}
...
export interface AssessResult {
  ...
  summary: { total: number; passed: number; failed: number; notEvaluated: number; failedBySeverity: Record<string, number> };
}
...
export function assess(findings: Finding[], context?: { providerRuns: ProviderRun[] }): AssessResult {
  ...
  const results = ASSESS_CHECKS.map<AssessCheckResult>((check) => {
    ...
    if (matched.length === 0 && unavailable.length > 0) {
      ...
      return {
        ...
        severity: null,
        findingIds: [],
        reason: `Required observation incomplete: ${unavailable.join(", ")}.`,
      };
    }
    if (matched.length === 0) {
      passed += 1;
  ...
  return {
    ...
    results,
    summary: { total: results.length, passed, failed, notEvaluated, failedBySeverity },
  };
}
