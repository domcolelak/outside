...
function assetsWithSignal(result: ScanResult, code: string): Asset[] {
  return result.graph.assets.filter((a) =>
    a.signals.some((s) => s.code === code && s.confidence >= 0.55),
  );
}
...
const genMailSecurity: Gen = ({ result, now }) => {
  const mail = result.graph.assets.find(
    (a) => a.kind === "mail_service" && a.attrs.spf === "missing",
  );
  if (!mail) return null;
  ...
  return {
    ...
    businessImpact:
      "Attackers can send email that appears to come from your domain, targeting staff, customers, and partners.",
    assetIds: [mail.id],
    scoreComponentCode: "mail",
    estimatedReduction: reduction,
    remediation: {
      summary:
        "Publish SPF, then DKIM, then a DMARC policy (start at p=none and monitor before enforcing).",
      mode: "guided",
      changesInfrastructure: true,
      connector: "dns",
      steps: [
        {
          instruction: "Publish an SPF TXT record",
          detail: 'e.g. "v=spf1 include:_spf.your-provider.com -all"',
        },
        {
          instruction:
            "Enable DKIM signing at your mail provider and publish the DKIM public key",
        },
        {
          instruction: "Publish a DMARC record at _dmarc",
          detail: '"v=DMARC1; p=none; rua=mailto:dmarc@yourdomain"',
        },
        {
          instruction:
            "Monitor DMARC aggregate reports, then move p=none → quarantine → reject",
        },
      ],
      rollback:
        "Remove or revert the TXT records; DNS changes are non-destructive and fully reversible.",
...
const genNonProd: Gen = ({ result, now }) => {
  ...
  return {
    ...
    businessImpact:
      "Non-production systems frequently expose test data, verbose errors, or default credentials that don't belong on the public internet.",
    assetIds: assets.map((a) => a.id),
    scoreComponentCode: "nonprod",
    estimatedReduction: penaltyOf(result, "nonprod"),
    remediation: {
      summary:
        "Put non-production environments behind an IP allowlist, VPN, or authentication — or take them offline.",
      mode: "guided",
      changesInfrastructure: true,
      ...
      steps: [
        {
          instruction:
            "Confirm each environment is intended to be internet-facing",
        },
        {
          instruction:
            "Apply an access policy (Cloudflare Access / WAF rule / IP allowlist) or require SSO",
        },
        { instruction: "Decommission environments that are no longer needed" },
      ],
      rollback:
        "Access policies can be disabled instantly, restoring the prior state.",
    },
    status: "open",
    createdAt: now,
  };
...
const genShadow: Gen = ({ result, now }) => {
  ...
  return {
    ...
    confidence: Math.max(
      ...assets.map(
        (a) =>
          a.signals.find((s) => s.code === "asset.shadow")?.confidence ?? 0.6,
      ),
    ),
    ...
    evidence: firstEvidence(assets, []),
    businessImpact:
      "Forgotten infrastructure is rarely patched or monitored, making it a common entry point that no one is watching.",
    assetIds: assets.map((a) => a.id),
    scoreComponentCode: "shadow",
    estimatedReduction: penaltyOf(result, "shadow"),
    remediation: {
      summary:
        "Confirm ownership and business purpose for each asset; decommission what isn't needed and bring the rest under management.",
      mode: "guided",
      changesInfrastructure: false,
      steps: [
        {
          instruction:
            "Identify the owning team and current purpose of each asset",
        },
        {
          instruction:
            "Decommission assets with no owner or purpose (remove DNS + shut down origin)",
        },
        {
          instruction:
            "For assets that must stay, add them to your inventory, patching, and monitoring",
        },
      ],
      rollback:
        "Inventory/monitoring changes are reversible; decommissioning should follow your change process.",
    },
    status: "open",
    createdAt: now,
  };
...
const genAuthSurface: Gen = ({ result, now }) => {
  ...
  return {
    ...
    evidence: firstEvidence(assets, []),
    businessImpact:
      "Every public login is an attack surface for password spraying, credential stuffing, and exploitation of the auth stack.",
    assetIds: assets.map((a) => a.id),
    scoreComponentCode: "auth",
    estimatedReduction: penaltyOf(result, "auth"),
    remediation: {
      summary:
        "Confirm each surface must be public; enforce MFA, rate limiting, and monitoring, and restrict to known networks where possible.",
      mode: "guided",
      changesInfrastructure: true,
      connector: "cloudflare",
      steps: [
        ...
        {
          instruction:
            "Enforce MFA and rate limiting; enable brute-force lockout",
        },
        {
          instruction:
            "Restrict to corporate networks or an access proxy where feasible",
        },
      ],
...
const genSurfaceChange: Gen = ({ result, now }) => {
  const events =
    result.changeSummary?.events.filter(
      (e) => e.type === "asset_appeared" || e.type === "asset_returned",
    ) ?? [];
  if (events.length === 0) return null;
  const ids = events
    .map(
      (e) => result.graph.assets.find((a) => a.canonical === e.canonical)?.id,
    )
  ...
  return {
    ...
    evidence: [],
    businessImpact:
      "Unreviewed surface growth is how shadow IT and forgotten deployments accumulate risk unnoticed.",
    assetIds: ids,
    scoreComponentCode: "new",
    estimatedReduction: penaltyOf(result, "new"),
    remediation: {
      summary:
        "Confirm each change was intentional and owned by a known team; add to inventory or decommission.",
      mode: "guided",
      changesInfrastructure: false,
      steps: [
        {
          instruction:
            "Confirm each new/returned asset was published intentionally",
        },
...
const genThirdParty: Gen = ({ result, now }) => {
  ...
  return {
    ...
    evidence: firstEvidence(assets, []),
    businessImpact:
      "A compromised or misconfigured third-party service can be leveraged against your domain (e.g. subdomain takeover).",
    assetIds: assets.map((a) => a.id),
    estimatedReduction: 0,
    remediation: {
      summary:
        "Inventory third-party delegations and watch for dangling CNAMEs (subdomain-takeover risk).",
      mode: "guided",
      changesInfrastructure: false,
      steps: [
        {
        ...
        {
          instruction:
            "Remove DNS records that point to deprovisioned services (dangling CNAMEs)",
        },
        {
          instruction:
            "Track each provider's security posture and incident notifications",
        },
      ],
...
const genSecurityHeaders: Gen = ({ result, now }) => {
  ...
  const missing =
    (observed?.attrs.missingHeaders as string[] | undefined) ?? [];
  ...
  return {
    ...
    priority: "medium",
    confidence: 0.95,
    why: `${observed.label} responded without baseline security headers: ${missing.join(", ")}.`,
    evidence: observed.evidence.slice(0, 2),
    businessImpact:
      "Missing headers leave the site more exposed to clickjacking, MIME sniffing, protocol downgrade, and referrer leakage.",
    assetIds: [observed.id],
    scoreComponentCode: "headers",
    estimatedReduction: penaltyOf(result, "headers"),
    remediation: {
      summary:
        "Add the missing response headers at your edge/CDN or web server.",
      mode: "guided",
      changesInfrastructure: true,
      connector: "cloudflare",
      steps: [
        {
          instruction: "Enable HSTS",
          detail:
            "Strict-Transport-Security: max-age=31536000; includeSubDomains",
        },
        {
          instruction:
            "Set X-Content-Type-Options: nosniff and a X-Frame-Options / frame-ancestors policy",
        },
        {
          instruction:
            "Add a Content-Security-Policy (start report-only) and a Referrer-Policy",
        },
      ],
      rollback:
        "Headers can be removed instantly at the edge with no data impact.",
...
const genCertLifecycle: Gen = ({ result, now }) => {
  const asset = selectPrimaryWebSurface(
    result.graph.assets,
    result.target,
    (candidate) =>
      hasCertificateObservation(candidate) &&
      (candidate.attrs.certDaysToExpiry as number) < 21,
  )?.asset;
  ...
  return {
    ...
    title:
      days < 0
        ? `Renew the expired certificate on ${asset.label}`
        : `Certificate on ${asset.label} expires in ${days} days`,
    ...
    businessImpact:
      "An expired certificate breaks HTTPS for every visitor and erodes trust; last-minute renewals are a common outage cause.",
    assetIds: [asset.id],
    scoreComponentCode: "cert_expiry",
    estimatedReduction: penaltyOf(result, "cert_expiry"),
    remediation: {
      summary:
        "Renew the certificate and enable automated renewal so this never becomes urgent again.",
      mode: "guided",
      changesInfrastructure: true,
      connector: "acme",
      steps: [
        ...
        {
          instruction:
            "Enable automated renewal (ACME / your CDN's managed certificates)",
        },
        {
          instruction:
            "Add expiry monitoring so you're alerted well before 21 days",
        },
      ],
      rollback:
        "Certificate renewal is additive; the prior certificate remains valid until its expiry.",
    },
    status: "open",
    createdAt: now,
  };
...
export function buildPosture(result: ScanResult): Posture {
  ...
  const ctx: Ctx = {
    result,
    now,
    assetsByCanon: new Map(result.graph.assets.map((a) => [a.canonical, a])),
  };
  ...
  const potentialScore = Math.max(
    0,
    Math.min(100, currentScore + openReduction),
  );

  const openByPriority: Posture["openByPriority"] = {
    critical: 0,
    high: 0,
    medium: 0,
    low: 0,
  ...
  for (const r of recommendations)
    if (r.status === "open") openByPriority[r.priority] += 1;
