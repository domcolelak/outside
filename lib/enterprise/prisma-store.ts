...
export class PrismaEnterpriseStore implements EnterpriseStore {
  ...
  async provisionScimUserAtomic(input: { workspaceId: string; orgId: string; providerId: string; directoryUserId?: string | null; feature: "scim" | "sso"; email: string; name: string; passwordHash: string; externalId: string | null; active: boolean; attributes?: Record<string, unknown> }, audit: Omit<AppendEnterpriseAuditInput, "workspaceId" | "resourceId">) {
    return serializableTransaction(async (transaction) => {
      const tx = transaction as unknown as EnterpriseDb;
      const rawWorkspace = await tx.enterpriseWorkspace.findUnique({ where: { id: input.workspaceId } });
      const workspace = rawWorkspace ? record<EnterpriseWorkspace>(rawWorkspace) : null;
      const rawProvider = (await tx.enterpriseIdentityProvider.findMany({ where: { id: input.providerId, workspaceId: input.workspaceId, enabled: true }, take: 1 }))[0];
      if (!workspace || workspace.orgId !== input.orgId || !rawProvider || !workspaceInRegion(workspace) || !featureEnabled(workspace, input.feature)) throw new Error("Enterprise provisioning is not available for this workspace.");
      const rawExisting = (await tx.enterpriseDirectoryUser.findMany({ where: { workspaceId: input.workspaceId, userName: input.email }, take: 1 }))[0];
      const existing = rawExisting ? record<import("./types").EnterpriseDirectoryUser>(rawExisting) : null;
      if (existing && (!input.directoryUserId || existing.id !== input.directoryUserId)) throw new Error("SCIM user already exists.");
      if (input.active && !existing?.active && await tx.enterpriseDirectoryUser.count({ where: { workspaceId: input.workspaceId, active: true } }) >= workspace.licensedSeats) throw new Error("Enterprise licensed seat limit reached.");
      ...
      const data = { userId: user.id, externalId: input.externalId, displayName: input.name, active: input.active, attributes: input.attributes ?? existing?.attributes ?? {}, lastSyncedAt: new Date() };
      const rawItem = existing
        ? (await tx.enterpriseDirectoryUser.updateMany({ where: { id: existing.id, workspaceId: input.workspaceId, identityProviderId: input.providerId }, data })).count === 1
          ? await tx.enterpriseDirectoryUser.findUnique({ where: { id: existing.id } })
          : null
        : await tx.enterpriseDirectoryUser.create({ data: { ...data, id: `directoryUsers_${randomUUID()}`, workspaceId: input.workspaceId, identityProviderId: input.providerId, userName: input.email, departmentId: null } });
      if (!rawItem) throw new Error("Enterprise directory user could not be persisted.");
      const item = record<import("./types").EnterpriseDirectoryUser>(rawItem);
  ...
  async updateScimUserAtomic(input: { workspaceId: string; orgId: string; providerId: string; id: string; patch: Partial<import("./types").EnterpriseDirectoryUser> }, audit: Omit<AppendEnterpriseAuditInput, "workspaceId" | "resourceId">) {
    return serializableTransaction(async (transaction) => {
      ...
      const workspace = rawWorkspace ? record<EnterpriseWorkspace>(rawWorkspace) : null;
      const rawProvider = (await tx.enterpriseIdentityProvider.findMany({ where: { id: input.providerId, workspaceId: input.workspaceId, enabled: true }, take: 1 }))[0];
      const rawCurrent = (await tx.enterpriseDirectoryUser.findMany({ where: { id: input.id, workspaceId: input.workspaceId, identityProviderId: input.providerId }, take: 1 }))[0];
      const current = rawCurrent ? record<import("./types").EnterpriseDirectoryUser>(rawCurrent) : null;
      if (!workspace || workspace.orgId !== input.orgId || !rawProvider || !current || !workspaceInRegion(workspace) || !featureEnabled(workspace, "scim")) return null;
      if (input.patch.active === true && !current.active && await tx.enterpriseDirectoryUser.count({ where: { workspaceId: input.workspaceId, active: true } }) >= workspace.licensedSeats) throw new Error("Enterprise licensed seat limit reached.");
  ...
  async renewDeliveryLease(workspaceId: string, id: string, leaseId: string, now: Date, leaseMs: number) { return (await db.enterpriseDelivery.updateMany({ where: { id, workspaceId, status: "processing", leaseId }, data: { leasedUntil: new Date(now.getTime() + leaseMs) } })).count === 1; }
