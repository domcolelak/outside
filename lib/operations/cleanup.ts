...
export async function runOperationalCleanup(now = new Date(), batchSize = 5000): Promise<OperationalCleanupResult> {
  if (!process.env.DATABASE_URL || process.env.OUTSIDE_STORAGE_MODE === "memory") return { rateLimits: 0, concurrencyLeases: 0, emailOutbox: 0, processedEvents: 0, usageEvents: 0, passwordResets: 0, scanShares: 0 };
  ...
  const [rateLimits, concurrencyLeases, emailOutbox, processedEvents, usageEvents, passwordResets, scanShares] = await prisma.$transaction([
  ...
  return { rateLimits, concurrencyLeases, emailOutbox, processedEvents, usageEvents, passwordResets, scanShares };
}
