...
/** Bounded retention for operational and explicitly time-bounded product data. */
export async function runOperationalCleanup(now = new Date(), batchSize = 5000): Promise<OperationalCleanupResult> {
  ...
  const assessmentCutoff = new Date(now.getTime() - boundedDays(process.env.OUTSIDE_ASSESSMENT_RETENTION_DAYS, 730, 90) * 86_400_000);
  ...
  const [
    ...
    prisma.$executeRaw`WITH doomed AS (SELECT ctid FROM provider_usage_events WHERE "createdAt" < ${usageCutoff} ORDER BY "createdAt" LIMIT ${limit}) DELETE FROM provider_usage_events WHERE ctid IN (SELECT ctid FROM doomed)`,
    prisma.$executeRaw`WITH doomed AS (SELECT ctid FROM assessment_runs WHERE "createdAt" < ${assessmentCutoff} ORDER BY "createdAt" LIMIT ${limit}) DELETE FROM assessment_runs WHERE ctid IN (SELECT ctid FROM doomed)`,
    ...
    prisma.$executeRaw`WITH doomed AS (SELECT ctid FROM scan_shares WHERE "expiresAt" < ${now} ORDER BY "expiresAt" LIMIT ${limit}) DELETE FROM scan_shares WHERE ctid IN (SELECT ctid FROM doomed)`,
  ]);
  return {
    rateLimits,
    concurrencyLeases,
