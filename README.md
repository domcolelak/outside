...
**Live demo: [outsideguardian.eu](https://outsideguardian.eu)** — start with an anonymous external snapshot, then verify ownership to unlock continuous Guardian monitoring (`/scan?target=northstar&mode=demo`).

OUTSIDE is continuous external exposure management for teams and agencies. Give it one company domain and the free snapshot maps the public assets and signals it can verify from that starting point; verification then unlocks scheduled monitoring, change intelligence, Guardian recommendations, reports, remediation guidance, and one controlled Cloudflare DMARC action. It separates what was _observed_ from what was _inferred_, cites the evidence behind every claim, and refuses to report anything the deterministic pipeline did not see.
...
| **A free snapshot starts the continuous lifecycle** — discover, verify, monitor and act. The panel on the right replays a real discovery sequence; verified ownership then unlocks scheduled Guardian monitoring and change intelligence.                                              | **The live scan** — the discovery sequence on the left is what the product is doing right now; the graph is the shared mental model; the right rail carries protection posture, statistics and Aegis incident correlation. |
...
- Optional third-party threat-intelligence enrichment — IP reputation (AbuseIPDB), breach exposure (HaveIBeenPwned), IP classification (GreyNoise), and domain reputation (VirusTotal) — runs only on verified targets. Organization administrators can connect encrypted BYOK credentials under `/integrations`; explicitly licensed platform keys remain an operator fallback only when that provider is not connected. An unreadable connected credential fails closed and is never silently replaced or metered as BYOK.
- Every external key the product uses can be the organization's own: HaveIBeenPwned, SecurityTrails, Shodan, AbuseIPDB, GreyNoise, VirusTotal, Censys and OpenAI all connect through one encrypted store, one connection test, one usage counter and one audit trail. Censys is the pair case — an API ID and secret are stored as a single encrypted credential and expanded only where the scan needs them.
...
- `lib/observability`: low-cardinality OpenTelemetry metrics for discovery providers, required scan persistence, Guardian queue age/depth, delivery outcomes, and retention.
