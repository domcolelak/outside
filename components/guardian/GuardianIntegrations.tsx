...
> = {
  ...
  microsoft_teams: [
    { key: "url", label: "Workflow webhook URL", secret: true },
  ],
  discord: [{ key: "url", label: "Incoming webhook URL", secret: true }],
  webhook: [
    { key: "url", label: "HTTPS endpoint" },
  ...
  github_issues: [
    ...
    { key: "token", label: "Fine-grained token", secret: true },
  ],
  linear: [
...
export function GuardianIntegrations({
  orgId,
  initialChannels,
  canAdmin,
}: {
  ...
  const add = async (event: React.FormEvent) => {
    ...
    setError(null);
    const response = await fetch("/api/guardian/channels", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ orgId, type, name, config }),
    });
  ...
  const toggle = async (channel: GuardianChannel) => {
    const response = await fetch(
      `/api/guardian/channels/${channel.id}?orgId=${encodeURIComponent(orgId)}`,
      {
        method: "PATCH",
        headers: { "content-type": "application/json" },
    ...
    if (response.ok)
      setChannels((rows) =>
        rows.map((row) =>
          row.id === channel.id ? { ...row, enabled: !row.enabled } : row,
        ),
      );
  };
  ...
  return (
    <div className="panel p-5 md:p-6">
      ...
      {adding && (
        <form
          onSubmit={add}
          className="mt-5 rounded-xl border border-line bg-base-950/70 p-4"
        >
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="text-xs text-ink-faint">
              ...
              <select
                value={type}
                onChange={(event) => {
                  setType(event.target.value as GuardianChannelType);
                  setConfig({});
                }}
                className="mono mt-1.5 w-full rounded-lg border border-line bg-base-900 px-3 py-2.5 text-xs text-ink"
              >
            ...
            <label className="text-xs text-ink-faint">
              ...
              <input
                ...
                maxLength={80}
                className="mt-1.5 w-full rounded-lg border border-line bg-base-900 px-3 py-2.5 text-xs text-ink outline-hidden focus:border-signal/30"
              />
            </label>
            {fields[type].map((field) => (
              ...
              >
                ...
                <input
                  ...
                  className="mono mt-1.5 w-full rounded-lg border border-line bg-base-900 px-3 py-2.5 text-xs text-ink outline-hidden placeholder:text-ink-faint focus:border-signal/30"
                />
              </label>
            ))}
          </div>
      ...
      <div className="mt-5 space-y-2">
        {channels.map((channel) => (
          <div
            key={channel.id}
            className="flex items-center justify-between gap-3 rounded-xl border border-line bg-base-950/50 p-3"
          >
            <div className="flex min-w-0 items-center gap-3">
              <div
                className={`grid h-9 w-9 shrink-0 place-items-center rounded-lg border text-xs font-semibold ${channel.enabled ? "border-signal/20 bg-signal/5 text-signal" : "border-line text-ink-faint"}`}
              >
                {labels[channel.type].slice(0, 2).toUpperCase()}
              </div>
