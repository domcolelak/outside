...
export function AttackerView({ result, onClose, autoPresent = false }: { result: ScanResult; onClose: () => void; autoPresent?: boolean }) {
  ...
  return <div className="fixed inset-0 z-50 flex flex-col overflow-hidden bg-base-950">
    ...
    <footer data-capture-hide className="relative z-30 flex items-center gap-3 border-t border-line bg-base-950/88 px-4 py-3 backdrop-blur-xl md:px-6"><button onClick={() => finished ? restart() : setPlaying((value) => !value)} className="mono grid h-9 w-9 place-items-center rounded-xl border border-signal/25 bg-signal/6 text-[10px] text-signal">{finished ? "↻" : playing ? "Ⅱ" : "▶"}</button><input aria-label="Replay position" type="range" min={0} max={totalDuration} step={.05} value={elapsed} onChange={(event) => { seek(Number(event.target.value)); setPlaying(false); }} className="h-1 flex-1 cursor-pointer accent-signal"/><span className="mono w-20 text-right text-[9px] text-ink-faint">{elapsed.toFixed(1)} / {totalDuration.toFixed(0)}s</span><div className="hidden items-center gap-1 sm:flex">{[.75,1,1.5].map((value) => <button key={value} onClick={() => setSpeed(value)} className={`mono rounded-md px-2 py-1 text-[8px] ${speed === value ? "bg-signal/10 text-signal" : "text-ink-faint"}`}>{value}×</button>)}</div><button onClick={() => setShowTimeline((value) => !value)} className="mono hidden rounded-lg border border-line px-2.5 py-2 text-[8px] text-ink-faint lg:block">{showTimeline ? "Hide steps" : "Show steps"}</button></footer>
  </div>;
}
