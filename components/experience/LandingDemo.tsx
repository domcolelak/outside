...
export function LandingDemo() {
  ...
  return <div className="relative w-full max-w-[620px] overflow-hidden rounded-[22px] border border-line-strong bg-base-950/82 shadow-[0_40px_140px_-50px_rgba(56,225,195,.25)] backdrop-blur-xl">
    <div className="flex items-center justify-between border-b border-line px-4 py-3"><div className="flex items-center gap-2"><span className="relative flex h-2 w-2"><span className="absolute h-full w-full animate-ping rounded-full bg-signal opacity-30"/><span className="relative h-2 w-2 rounded-full bg-signal"/></span><span className="mono text-[11px] uppercase tracking-[.18em] text-signal">Live product story</span></div><div className="mono text-[11px] text-ink-faint">NORTHSTAR LABS · DEMO</div></div>
    <div className="relative aspect-16/10 min-h-[360px]">
      ...
      <div className="absolute left-4 top-4 rounded-lg border border-line bg-base-950/72 px-3 py-2 backdrop-blur-sm"><div className="mono text-[10px] uppercase text-ink-faint">External surface</div><div className="mt-1 text-lg font-medium text-ink">{current.metric}</div></div>
    ...
    <div className="flex items-center gap-3 border-t border-line px-4 py-3"><button onClick={() => setPlaying((value) => !value)} aria-label={playing ? "Pause story" : "Play story"} className="mono grid h-7 w-7 place-items-center rounded-lg border border-line text-[11px] text-ink-soft hover:border-signal/30 hover:text-signal">{playing ? "Ⅱ" : "▶"}</button><div className="flex flex-1 gap-1">{scenes.map((item, index) => <button key={item.label} onClick={() => { setScene(index); setPlaying(false); }} className="group flex min-h-7 flex-1 items-center" aria-label={`Show ${item.label}`} aria-current={index === scene ? "step" : undefined}><span className={`block h-1 w-full rounded-full transition ${index <= scene ? "bg-signal" : "bg-base-600"}`}/><span className={`mono mt-1 hidden text-[7px] uppercase md:block ${index === scene ? "text-signal" : "text-ink-faint"}`}>{item.label}</span></button>)}</div><Link href="/scan?target=northstar&mode=demo&present=1" className="mono rounded-lg bg-signal px-3 py-2 text-[11px] font-semibold uppercase text-base-950">Open demo</Link></div>
  </div>;
}
