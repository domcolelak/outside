...
export function Walkthrough({ steps, open, onClose }: { steps: WalkthroughStep[]; open: boolean; onClose: () => void }) {
  ...
  return <div className="fixed inset-x-0 bottom-6 z-[70] mx-auto w-[min(92vw,620px)] animate-rise-in" role="dialog" aria-label="Product walkthrough"><div className="rounded-2xl border border-signal/20 bg-base-950/95 p-4 shadow-[0_30px_100px_rgba(0,0,0,.7)] backdrop-blur-2xl"><div className="flex items-start gap-4"><div className="mono grid h-9 w-9 shrink-0 place-items-center rounded-xl border border-signal/20 bg-signal/10 text-[10px] text-signal">{String(index + 1).padStart(2, "0")}</div><div className="min-w-0 flex-1"><div className="mono text-[9px] uppercase tracking-[.18em] text-signal">{step.eyebrow}</div><div className="mt-1 text-base font-medium text-ink">{step.title}</div><p className="mt-1 text-xs leading-5 text-ink-soft">{step.body}</p></div><button onClick={onClose} aria-label="Close walkthrough" className="text-ink-faint hover:text-ink">×</button></div><div className="mt-4 flex items-center justify-between"><div className="flex gap-1">{steps.map((_, item) => <button key={item} onClick={() => setIndex(item)} aria-label={`Step ${item + 1}`} className={`h-1 rounded-full transition-all ${item === index ? "w-8 bg-signal" : "w-3 bg-base-600"}`}/>)}</div><div className="flex gap-2">{index > 0 && <button onClick={() => setIndex((value) => value - 1)} className="rounded-lg border border-line px-3 py-1.5 text-[10px] text-ink-soft">Back</button>}<button onClick={next} className="rounded-lg bg-signal px-3 py-1.5 text-[10px] font-semibold text-base-950">{index === steps.length - 1 ? "Finish" : "Next"}</button></div></div></div></div>;
}
