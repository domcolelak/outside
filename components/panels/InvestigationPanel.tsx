  ...
  return (
    <div>
      ...
      <div className="panel p-4">
        ...
        {/* Correlation chain */}
        <div className="mt-3">
          <div className="mono text-[10px] uppercase tracking-wide text-ink-faint">Correlation chain</div>
        ...
        {assessment && (
          <div className="mt-3 space-y-3 border-t border-line pt-3">
            ...
            <div className="rounded-lg border border-risk-medium/25 bg-risk-medium/5 p-2.5">
              <div className="mono mb-1 text-[10px] uppercase tracking-wide text-risk-medium">Counter-evidence (Devil&apos;s Advocate)</div>
              <ul className="space-y-1">
                {assessment.contradictingEvidence.map((c, i) => (
                  <li key={i} className="flex gap-2 text-[11px] leading-snug text-ink-soft">
...
function MoreIncidents({ incidents }: { incidents: ExposureIncident[] }) {
  ...
  return (
    <div className="mt-3 border-t border-line pt-3">
      <button onClick={() => setOpen((v) => !v)} className="mono text-[11px] text-signal hover:underline">
        {open ? "Hide" : `${incidents.length} more correlated incident${incidents.length > 1 ? "s" : ""}`}
      </button>
      {open && (
        <div className="mt-2 space-y-1.5">
          {incidents.map((inc) => (
            <div key={inc.id} className="flex items-center gap-2 text-[11px] text-ink-soft">
