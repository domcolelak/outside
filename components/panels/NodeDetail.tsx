...
import { AssuranceTag, Chip, Confidence, PriorityDot } from "@/components/ui";
import { localizeAssetKind, localizeDiscoveryMethod, localizeSignal } from "@/lib/report/scan-copy";
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="relative overflow-hidden border-b border-line px-4 py-4"><div className="pointer-events-none absolute -right-12 -top-16 h-36 w-36 rounded-full bg-signal/10 blur-3xl"/><div className="relative flex items-start justify-between gap-3">
        <div>
          <div className="mono text-xs uppercase tracking-wider text-ink-faint">{localizeAssetKind(asset.kind, tr)}</div>
      ...
      <div className="scroll-thin flex-1 space-y-4 overflow-y-auto px-4 py-4 text-sm">
        <div className="flex flex-wrap items-center gap-2">
          <span className="inline-flex items-center gap-2 rounded-md border border-line px-2 py-1">
            ...
            <span className="text-xs text-ink-soft">{tr.t("ui", "reviewPriority", { priority: tr.t("ui", `priority${asset.priority[0]!.toUpperCase()}${asset.priority.slice(1)}` as Parameters<typeof tr.t<"ui">>[1]) })}</span>
          </span>
          {protocols.map((p) => (
            <Chip key={p}>{p}</Chip>
          ))}
        ...
        <div className="rounded-xl border border-line bg-base-950/50 p-3"><div className="mono text-[11px] uppercase tracking-wider text-ink-faint">{s("assetLens")}</div><div className="mt-3 grid grid-cols-3 gap-2 text-center">{[[asset.evidence.length,s("evidenceHeading")],[asset.signals.length,s("signalsHeading")],[asset.discoveredVia.length,s("discoveryPaths")]].map(([value,label]) => <div key={label} className="rounded-lg bg-base-900 p-2"><div className="text-lg font-medium text-ink">{value}</div><div className="mono text-[10px] uppercase text-ink-faint">{label}</div></div>)}</div></div>
        ...
        <Section title={s("orgConfidence")}>
          ...
          <p className="mt-1 text-xs text-ink-faint">{tr.t("ui", "assetOwnershipConfidence")}</p>
        </Section>
        ...
        {asset.signals.length > 0 && (
          <Section title={s("signalsHeading")}>
            <div className="space-y-2">
              {asset.signals.map((signal) => { const copy = localizeSignal(signal, asset, tr); return (
                <div key={signal.code} className="rounded-lg border border-line bg-base-850 p-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <span className="text-[13px] text-ink">{copy.label}</span>
                    <AssuranceTag assurance={signal.assurance} />
                  </div>
                  <div className="mt-2"><Confidence value={signal.confidence} /></div>
                  <p className="mt-2 text-xs leading-relaxed text-ink-soft">{copy.rationale}</p>
                </div>
              ); })}
            </div>
          </Section>
            ...
            {asset.evidence.map((e, i) => (
              <div key={i} className="motion-card relative rounded-lg border border-line bg-base-850 p-3 pl-10">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="mono text-[11px] text-ink-faint">{localizeDiscoveryMethod(e.method, tr)}</span>
                </div>
                <p className="mt-1.5 text-xs text-ink-soft">{tr.t("ui", "deterministicProviderObservation")}</p>
              </div>
            ))}
          </div>
        </Section>
        ...
        <Section title={s("discoveryHeading")}>
          <div className="flex flex-wrap items-center gap-1.5">
            {asset.discoveredVia.map((m, index) => (
              <span key={m} className="contents">{index > 0 && <span className="text-ink-faint">→</span>}<Chip tone="signal">{localizeDiscoveryMethod(m, tr)}</Chip></span>
            ))}
          </div>
