...
type Portal = {
  ...
  recommendations: Array<{ id: string; title: string; priority: string; why: string; businessImpact: string; suggestedReview: string }>;
  notes: Array<{ id: string; body: string; createdAt: string }>;
  reports: Array<{ id: string; title: string; kind: string; createdAt: string }>;
  recentChanges: Array<{ id: string; title: string; summary: string; observedAt: string }>;
};
...
export function ClientPortal() {
  ...
  useEffect(() => { fetch(`/api/agency/portal?agencyId=${encodeURIComponent(query.get("agencyId") ?? "")}&clientId=${encodeURIComponent(query.get("clientId") ?? "")}`).then(async (response) => { const result = await response.json(); if (!response.ok) throw new Error(result.error); setData(result); }).catch((reason: Error) => setError(reason.message)); }, [query]);
  ...
  return <div className="space-y-6" style={style}>
    ...
    <section className="grid gap-6 lg:grid-cols-2"><div className="panel p-5"><div className="mono text-[11px] uppercase text-ink-faint">{g("portalSharedItems")}</div><div className="mt-4 space-y-3">{data.recommendations.map((recommendation) => <article key={recommendation.id} className="rounded-xl border border-line p-4"><div className="flex justify-between gap-3"><h2 className="text-sm text-ink">{recommendation.title}</h2><span className="mono text-[11px] uppercase text-risk-high">{recommendation.priority}</span></div><p className="mt-2 text-xs leading-5 text-ink-soft">{recommendation.why}</p><p className="mt-2 text-[12px] text-ink-faint">{g("portalBusinessImpact", { impact: recommendation.businessImpact })}</p><p className="mt-1 text-[12px] text-ink-faint">{g("portalReview", { review: recommendation.suggestedReview })}</p></article>)}{!data.recommendations.length && <p className="text-sm text-ink-faint">{g("portalNoShared")}</p>}</div></div><div className="panel p-5"><div className="mono text-[11px] uppercase text-ink-faint">{g("portalRecentChanges")}</div><div className="mt-4 space-y-3">{data.recentChanges.map((event) => <div key={event.id} className="border-b border-line pb-3"><div className="text-xs text-ink">{event.title}</div><div className="mt-1 text-[12px] text-ink-faint">{event.summary}</div></div>)}</div></div></section>
    <section className="grid gap-6 lg:grid-cols-2"><div className="panel p-5"><div className="mono text-[11px] uppercase text-ink-faint">{g("portalReports")}</div><div className="mt-4 space-y-2">{data.reports.map((report) => <a key={report.id} href={`/api/agency/reports/${report.id}?agencyId=${data.workspace.id}`} className="flex items-center justify-between rounded-lg border border-line p-3 text-sm hover:border-signal/30"><span>{report.title}</span><span className="mono text-[11px] text-ink-faint">{g("portalReportMeta", { date: tr.formatDate(report.createdAt) })}</span></a>)}{!data.reports.length && <p className="text-sm text-ink-faint">{g("portalNoReports")}</p>}</div></div><div className="panel p-5"><div className="mono text-[11px] uppercase text-ink-faint">{g("portalNotes")}</div><div className="mt-4 space-y-3">{data.notes.map((note) => <div key={note.id} className="rounded-lg border border-line p-3"><p className="text-sm leading-6 text-ink-soft">{note.body}</p><div className="mono mt-2 text-[11px] text-ink-faint">{tr.formatDate(note.createdAt, { dateStyle: "medium", timeStyle: "short" })}</div></div>)}{!data.notes.length && <p className="text-sm text-ink-faint">{g("portalNoNotes")}</p>}</div></div></section>
