...
export default async function CustomAgencyDomain({ searchParams }: { searchParams: Promise<{ domain?: string }> }) {
  ...
  if (!workspace?.branding.whiteLabel) return <main className="mx-auto max-w-xl px-6 py-24"><div className="panel p-8"><h1 className="text-2xl font-semibold">{g("portalUnavailable")}</h1><p className="mt-3 text-sm text-ink-soft">{g("portalNotConnected")}</p></div></main>;
  ...
  return <main className="mx-auto min-h-screen max-w-3xl px-6 py-20"><section className="panel p-8" style={{ borderTopColor: workspace.branding.primaryColor, borderTopWidth: 3 }}>{workspace.branding.logoUrl && <img src={workspace.branding.logoUrl} alt={`${workspace.name} logo`} referrerPolicy="no-referrer" decoding="async" className="mb-6 h-10 max-w-48 object-contain"/>}<h1 className="text-3xl font-semibold">{g("portalChooserTitle", { workspace: workspace.name })}</h1><p className="mt-2 text-sm text-ink-soft">{g("portalChooseWorkspace")}</p><div className="mt-6 grid gap-3">{accessible.map((client) => <Link key={client.id} href={`/agency/portal?agencyId=${workspace.id}&clientId=${client.id}`} className="rounded-lg border border-line p-4 hover:border-signal/30">{client.organizationName}</Link>)}{!accessible.length && <p className="text-sm text-ink-faint">{g("portalNoInvitation")}</p>}</div></section></main>;
}
