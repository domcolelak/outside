...
export async function GET(req: NextRequest) {
  ...
  if (!claim || !(await (await getAuthStore()).markEmailVerified(claim.uid, claim.email))) {
    return NextResponse.redirect(new URL("/account?emailVerification=invalid", appUrl));
  }
  return NextResponse.redirect(new URL("/account?emailVerification=complete", appUrl));
}
...
export async function POST(req: NextRequest) {
  ...
  await getEmailProvider().send(verifyEmail(ctx.user.email, `${appUrl}/api/auth/verify-email?token=${encodeURIComponent(token)}`));
