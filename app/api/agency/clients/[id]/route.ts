...
import { visibleAgencyNotes } from "@/lib/agency/visibility";
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ workspace: access.workspace, role: access.role, client, guardian, notes: visibleAgencyNotes(notes, access.role), shares, sla }, { headers: { "cache-control": "private, no-store" } });
}
