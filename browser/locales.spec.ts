...
const NONTRANSLATABLE =
  /^(OUTSIDE|Guardian|Attacker View|Chronos|Digital Twin|Evolution|Exposure Drift|Snapshot|Professional|Agency|Slack · Teams · Jira|Northstar Labs|Velora Commerce|Atlas Financial)$/;

test.describe("public experience in five languages", () => {
  for (const locale of LOCALES) {
    test(`renders the landing page in ${locale} without clipping or overflow`, async ({
      page,
    }) => {
      await page.goto("/");
      ...
      for (const width of [1440, 390]) {
        ...
        expect(
          overflow,
          `${locale} at ${width}px scrolls horizontally`,
        ).toBeLessThanOrEqual(1);

        const clipped = await page.evaluate(() =>
          [
            ...document.querySelectorAll(
              "h1, h2, h3, a, button, span, div, li, p",
            ),
          ]
            ...
            .map(
              (el) => `${el.tagName}: ${el.textContent?.trim().slice(0, 40)}`,
            ),
        );
  ...
  test("a visitor who picks Slovak gets Slovak copy, not English with a Slovak label", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  for (const locale of LOCALES) {
    test(`renders sign-in in ${locale} without clipping or overflow`, async ({
      page,
    }) => {
  ...
  test("a rejected sign-in explains itself in the chosen language", async ({
    page,
  }) => {
    ...
    await page
      .locator("form")
      .getByRole("button", { name: "Prihlásiť sa", exact: true })
      .click();
  ...
  test.describe("signed in", () => {
    ...
    test("billing and the Guardian paywall are written in the chosen language", async () => {
      ...
      for (const english of [
        ...
        expect(billing, `billing still shows "${english}"`).not.toContain(
          english,
        );
      }
      ...
      for (const english of [
        ...
        expect(guardian, `the paywall still shows "${english}"`).not.toContain(
          english,
        );
      }
    });
    ...
    test("Assess and Chronos are written in the chosen language", async () => {
      ...
      await expect(page.getByRole("heading", { level: 1 })).toHaveText(
        "Bezpečné, overené posúdenie zabezpečenia",
      );
      ...
      await expect(
        page.getByText("Platnosť a životnosť TLS certifikátu"),
      ).toBeVisible({ timeout: 20_000 });
      const assess = await page.locator("body").innerText();
      for (const english of [
        "Safe, verified security assessment",
        "Run assessment",
        "TLS certificate validity",
      ]) {
        expect(assess, `Assess still shows "${english}"`).not.toContain(
          english,
        );
      }
    ...
    test("the integrations page and its provider descriptions are translated", async () => {
      ...
      await expect(page.getByRole("heading", { level: 1 })).toHaveText(
        "Pripojte spravodajstvo a nápravu",
      );

      const body = await page.locator("body").innerText();
      expect(body).toContain(
        "Expozícia v únikoch pre domény, ktoré ste overili",
      );
      for (const english of [
        "Connect intelligence and remediation",
        "Read-only data sources",
        "Breach exposure for domains",
      ]) {
        expect(body, `integrations still shows "${english}"`).not.toContain(
          english,
        );
      }
    ...
    test("the capability registry is translated", async () => {
      ...
      await expect(page.getByRole("heading", { level: 1 })).toHaveText(
        "Čo dokáže OUTSIDE zistiť",
      );

      const body = await page.locator("body").innerText();
      expect(body).toContain("Objavovanie cez Certificate Transparency");
      for (const english of [
        "What OUTSIDE can detect",
        "Certificate Transparency discovery",
        "Always on",
        "Passive",
      ]) {
        expect(body, `capabilities still shows "${english}"`).not.toContain(
          english,
        );
      }
  ...
  test("the scan screen is written in the chosen language", async ({
    page,
  }) => {
    ...
    await expect(page.locator("body")).toContainText("Sprevádzaná prehliadka", {
      timeout: 20_000,
    });
    ...
    for (const english of [
      ...
      expect(body, `the scan screen still shows "${english}"`).not.toContain(
        english,
      );
    }
    expect(body).toContain("pohľad útočníka");
  });

  test("an unsupported language falls back to English instead of failing", async ({
    page,
  }) => {
    const rejected = await page.request.post("/api/locale", {
      data: { locale: "de" },
    });
