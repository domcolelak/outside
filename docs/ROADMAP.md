...
- One narrow write-capable action: Cloudflare DMARC monitor-mode TXT creation for a verified organization domain, with immutable preview binding, explicit approval, post-write verification, audit history, idempotency, and rollback.
- Deterministic known-vulnerability correlation over disclosed technology versions, enriched with the live CISA Known Exploited Vulnerabilities catalogue (exploited-in-the-wild status, ransomware linkage, federal remediation due dates) and EPSS 30-day exploitation-probability scores that sharpen prioritization.
...
- General provider-side remediation execution beyond the supported Cloudflare DMARC action. Other connectors currently provide credential registry, intelligence, or guided-preview capabilities only; no other provider mutation, post-change verifier, or automated rollback is implemented.
...
2. Add a reviewed connector adapter contract with least-privilege credentials, immutable previews, explicit approval, post-condition verification, and tested rollback before enabling any provider mutation beyond the current Cloudflare DMARC action.
