...
`lib/discovery/engine.ts` orchestrates a typed stage sequence shared by server and client. CT and DNS providers use bounded responses, deadlines, structured `ProviderRun` telemetry, and partial-success semantics; a scan reports its own completeness (`ScanCoverage`) so a partial discovery is never presented as whole. On verified targets, optional organization BYOK or explicitly licensed platform enrichment runs bounded and isolated — commercial passive-DNS (SecurityTrails, Shodan) and Censys service discovery expand the surface, while threat-intelligence (AbuseIPDB, HaveIBeenPwned, GreyNoise, VirusTotal) and vulnerability feeds (curated CVE set enriched by live CISA KEV + FIRST.org EPSS) annotate it. Provider credentials are validated before encrypted storage, connection status is cached until an explicit test, commercial-use boundaries fail closed where entitlement is detectable, and provider failure is captured rather than made fatal to the scan. A connected credential that cannot be decrypted is explicitly blocked for that request: it never falls through to a platform key, and usage is attributed only to organization credentials successfully loaded into the same scan context. Public CNAME suffixes provide explicit cloud/CDN signals; verified HTTPS responses add bounded literal Server/X-Powered-By technologies and recognized provider-header evidence. These remain observable signals rather than ownership claims. The maintained Public Suffix List supplies registrable-domain boundaries.
...
Evolution is a founder-only, observe-and-propose control loop rather than an autonomous code writer. Proposal decisions and detector incident feedback persist in PostgreSQL. Scheduled coverage runs and the set of proposal ids ever seen are also durable and serialized with a PostgreSQL advisory lock, so a deploy or overlapping scheduler cannot re-label an old gap as new. Bounded reliability factors learned from founder feedback are applied to future deterministic findings before scoring; a feedback-store outage is logged and the scan continues without adjustment. Approval produces a reviewable draft only — it never edits, commits, merges, deploys, or changes customer data.
...
Portfolio health derives from current protection-posture scores, unresolved recommendation priorities, observation freshness, and configured SLA windows. The persisted compatibility field remains `exposureScore`. No AI-generated value participates in authorization, evidence, scoring, or SLA status.
