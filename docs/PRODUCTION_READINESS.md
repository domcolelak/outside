...
| Unit/regression | `856` tests across `133` files, including auth/session, tenant isolation, SSRF/DNS rebinding, scoring/evidence, Guardian recovery, post-change verification of applied remediations, Stripe ordering, analytics privacy, export safety, caching, graph correctness, message-catalogue parity across five languages, and production configuration. The file count is asserted by `lib/docs/readiness.test.ts`, so this row cannot quietly go stale again. |
