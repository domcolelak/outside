...
while true; do
  if ! /opt/outside/backup.sh; then
    ...
    cat > "${metrics_directory}/outside_backup_failure.prom.tmp" <<EOF
...
EOF
    mv "${metrics_directory}/outside_backup_failure.prom.tmp" "${metrics_directory}/outside_backup_failure.prom"
